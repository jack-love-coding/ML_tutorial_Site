...
SHARED_REGRESSION_SOURCE = ROOT / "src/data/sharedRegressionTeachingData.ts"
...
def _validate_typescript_contract() -> None:
    ...
    shared_regression_text = SHARED_REGRESSION_SOURCE.read_text(encoding="utf-8")
    required = [
    ...
    for sample in REGRESSION_SAMPLES:
        ...
        if token not in text and token not in shared_regression_text:
...
def refresh_contract() -> None:
    """Refresh source-bound JSON without re-encoding unchanged published videos."""
    PUBLIC_DIR.mkdir(parents=True, exist_ok=True)
    write_if_changed(FIXTURE_PATH, json_bytes(checked_fixture()))
    for scene in SCENES:
        verify_video(PUBLIC_DIR / f"{scene['id']}.mp4", scene["durationSeconds"])
    write_if_changed(METADATA_PATH, json_bytes(metadata(collect_integrity())))
    print("Refreshed AI Overview shared-source contract without re-rendering videos.")
...
def main() -> int:
    ...
    parser.add_argument("--refresh-contract", action="store_true", help="Refresh source hashes and fixture JSON without re-rendering video")
    args = parser.parse_args()
    try:
        if args.check:
            check_all()
        elif args.refresh_contract:
            refresh_contract()
        else:
            render_all()
