...
During parameter comparison, use one line identity that transforms rather than seven simultaneously drawn lines. This prevents the learner from confusing candidate identity with a permanent model ensemble. Each leaderboard row is evidence tied to one state of that line. Reveal rows in committed order, even though the MSE sequence is not monotonic after the best candidate. That non-monotonic sequence helps show why the current candidate and current best are different notions. The GREEN box is applied only after all rows have appeared.
