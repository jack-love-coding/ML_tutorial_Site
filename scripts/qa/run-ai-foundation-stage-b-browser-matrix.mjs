...
export async function main() {
  ...
  try {
    ...
    await runCli(['open', 'http://127.0.0.1:4173/ML_tutorial_Site/courses/ai-foundation'])
    browserOpened = true
    await runCli(['run-code', '--filename', 'scripts/qa/aiFoundationStageBBrowserMatrix.js'])
    completed = true
  } finally {
    let cleanupError
