...
export const algorithmCheckpointsBySlug: Record<ModuleSlug, AlgorithmCheckpointItem[]> = {
  ...
  'python-notebook': [
    checkpoint(
      ...
      copy('工作日与非工作日的分组汇总出现差异时，哪一种解读最稳妥？', 'When grouped summaries differ between working and non-working days, which interpretation is most careful?'),
      [
        choice('observed-groups', '它描述当前快照中两组的观察差异，还需要结合分布和记录口径阅读。', 'It describes an observed difference between two groups in this snapshot and should be read with their distributions and recording rules.'),
        choice('every-hour', '它证明每一个工作日小时的租车数都高于非工作日。', 'It proves that every working-day hour has higher demand than every non-working-day hour.'),
        choice('cause', '它证明“是否工作日”单独造成了全部需求差异。', 'It proves that working-day status alone caused the entire demand difference.'),
      ],
      'observed-groups',
      copy(
        '分组汇总概括当前数据中的组间模式，不代表每条记录都遵循同一方向，也不能单独说明原因。应同时检查组内分布、样本数量和字段口径。',
        'A grouped summary describes a between-group pattern in the current data. It does not mean every record follows the same direction or establish a cause by itself. Read it with within-group distributions, group sizes, and field definitions.',
      ),
      ['group-summary-as-universal-rule', 'group-difference-as-cause'],
      'pandas-analysis',
    ),
    checkpoint(
      'python-data-tools-correlation-not-causation',
      copy('相关矩阵显示两个变量相关时，可以直接得到什么结论？', 'When a correlation matrix shows that two variables are correlated, what can you conclude directly?'),
      [
        choice('association-only', '当前快照中存在一定的线性关联；原因与方向仍需额外设计和背景信息。', 'The snapshot contains a degree of linear association; cause and direction still require additional design and context.'),
        choice('direct-cause', '相关系数较大的一方一定直接导致另一方变化。', 'The variable with the larger correlation necessarily causes the other to change.'),
        choice('complete-explanation', '只要相关系数不为零，就已经完整解释了业务机制。', 'Any nonzero correlation completely explains the real-world mechanism.'),
      ],
      'association-only',
      copy(
        'Pearson 相关描述线性共同变化，不排除共同原因、反向关系或聚合结构。最终报告应把它写成描述性发现，并明确“相关不代表因果”。',
        'Pearson correlation describes linear co-movement. It does not rule out shared causes, reverse direction, or aggregation effects. The final report should present it as a descriptive finding and state that correlation is not causation.',
      ),
      ['correlation-as-causation', 'correlation-as-complete-explanation'],
      'seaborn-statistics',
    ),
  ],
  ...
  classification: [
    ...
    checkpoint(
      ...
      copy('validation 选定成本阈值并锁定后，test 结果不如预期，应该怎么做？', 'After validation selects and freezes a cost-aware threshold, test is weaker than expected. What should you do?'),
      [
        choice('report-once', '如实报告一次 test 结果和 validation 折间波动，不回头重选阈值。', 'Report the one-time test result and validation fold variation without reselecting the threshold.'),
        choice('retune-test', '继续在 test 上扫描阈值，直到 test 指标最好。', 'Keep sweeping thresholds on test until its metric is best.'),
        choice('hide-errors', '只保留总体 AUC，不展示 test 或具名错误。', 'Keep only aggregate AUC and hide test plus named errors.'),
      ],
      'report-once',
      copy(
        'test 是最终泛化估计，不是第二个 validation。阈值选择证据、折间波动和具名 validation 错例应共同保留；test 只在锁定策略下汇总一次。',
        'Test is a final generalization estimate, not a second validation set. Preserve threshold evidence, fold variation, and named validation errors; summarize test once under the frozen policy.',
      ),
      ['test-threshold-reselection'],
      'biasCalibration',
    ),
  ],
  mlp: [
