...
const lessons: Record<HousingProjectChapterId, HousingProjectChapterLesson> = {
  ...
  'review-next-iteration': {
    id: 'review-next-iteration', blocks: [
      ...
      conclusion('review-conclusion', loc('这条回归项目链已经闭合：本地数据契约 → 训练集 EDA → 防泄漏流水线 → 诚实基线 → 验证选择 → 一次测试与失败复盘。监督学习走廊的下一站是逻辑回归：复用同样的切分纪律，把连续预测换成分数、概率和二分类损失；回到本项目时，再优先研究非线性地理特征与目标上限，而不是反复查看同一测试集。', 'The regression project loop is complete: local contract → training-only EDA → leakage-safe pipeline → honest baseline → validation selection → one test and failure review. The next supervised-learning stop is logistic regression: preserve the same split discipline while replacing continuous predictions with scores, probabilities, and binary loss. When returning to this project, study nonlinear geography and target capping instead of repeatedly inspecting the same test set.')),
    ],
    references: [
