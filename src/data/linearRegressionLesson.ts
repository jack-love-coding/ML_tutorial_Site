...
function lab(
  ...
  return { id, kind: 'observation-lab', title, prompt, controlLabels }
}
...
prediction(temp=0.50) = 156.042`, loc('单行预测可以直接拆成 16.610 + 278.863 × 0.50。斜率为正表示训练期内温度较高通常伴随更高需求。', 'The one-row prediction is 16.610 + 278.863 × 0.50. The positive slope says warmer hours in the training period tend to have higher demand.')),
      formula('fit-formula', loc('预测、斜率与截距', 'Prediction, slope, and intercept'), '$$\hat y=b+w_{temp}x_{temp}$$', loc('模型对每个输入只做乘法与加法。改变斜率会旋转直线，改变截距会整体上下平移直线。', 'The model only multiplies and adds. Changing the slope rotates the line; changing the intercept shifts it vertically.'), [
        { symbol: '$x_{temp}$', meaning: loc('一个小时的归一化温度', 'normalized temperature for one hour') },
        { symbol: '$w_{temp}$', meaning: loc('温度每增加 1 个归一化单位时预测的变化', 'prediction change for one normalized temperature unit') },
        { symbol: '$b$', meaning: loc('输入为 0 时直线的基准预测', 'baseline prediction when the input is zero') },
      ]),
      ...
      lab('fit-lab', loc('直线观察台', 'Line observation lab'), loc('改变样本温度或斜率倍率，观察预测怎样同步变化。', 'Change sample temperature or the slope multiplier and watch the prediction move with them.'), [loc('样本温度', 'Sample temperature'), loc('斜率倍率', 'Slope multiplier')]),
...
['temp', 'hum', 'windspeed', 'workingday', 'hr']`, loc('矩阵有 10,427 行、5 列。权重向量必须沿同一列顺序解释。', 'The matrix has 10,427 rows and 5 columns. The weight vector must be interpreted in the same order.')),
      formula('multi-formula', loc('从单行点积到整批矩阵', 'From one dot product to a batch matrix'), '$$\hat{\mathbf y}=X\mathbf w+b\mathbf 1$$', loc('X 的每一行是一个小时，每一列是一种特征；矩阵乘法一次产生整批预测。', 'Each row of X is one hour and each column is one feature; matrix multiplication produces the batch of predictions at once.'), [
        { symbol: '$X$', meaning: loc('形状为 n×p 的设计矩阵', 'n×p design matrix') },
        { symbol: '$\mathbf w$', meaning: loc('p 个特征权重', 'p feature weights') },
        { symbol: '$\hat{\mathbf y}$', meaning: loc('n 个预测值', 'n predictions') },
      ]),
      lab('multi-lab', loc('切分与特征观察台', 'Split and feature observation lab'), loc('切换单行/整批视图，核对时间边界、列顺序和被排除字段。', 'Switch between row and batch views to verify time boundaries, column order, and excluded fields.'), [loc('单行/整批', 'Row/batch'), loc('特征组', 'Feature group')]),
  ...
  'residual-loss': {
    ...
    blocks: [
      ...
      formula('loss-residual-formula', loc('先固定残差方向', 'Fix the residual direction first'), '$$r_i=\hat y_i-y_i$$', loc('本课统一使用 prediction − actual：正值表示预测偏高，负值表示预测偏低。', 'This course consistently uses prediction − actual: positive means overprediction, negative means underprediction.'), [
        { symbol: '$\hat y_i$', meaning: loc('第 i 行预测', 'prediction for row i') },
        { symbol: '$y_i$', meaning: loc('第 i 行实际租车量', 'actual rentals for row i') },
        { symbol: '$r_i$', meaning: loc('带方向的预测误差', 'signed prediction error') },
      ]),
...
R²   =     0.170`, loc('RMSE 与 cnt 使用同一单位，更容易回答“典型误差大约是多少辆”；R²=0.170 表示单温度模型只解释了有限变化。', 'RMSE uses the same unit as cnt, so it answers the scale of a typical error. R²=0.170 shows that temperature alone explains limited variation.')),
      ...
      lab('loss-lab', loc('残差与指标观察台', 'Residual and metric observation lab'), loc('切换残差案例和指标，观察大误差对 MSE、RMSE 与 MAE 的影响差异。', 'Switch residual cases and metrics to see how large errors affect MSE, RMSE, and MAE differently.'), [loc('残差案例', 'Residual case'), loc('指标', 'Metric')]),
...
max parameter delta vs sklearn = 2.7627e-08`, loc('NumPy 与 sklearn 的参数差约 10⁻⁸，说明实现和收敛条件一致；这不表示线性模型已经足够表达需求。', 'NumPy and sklearn differ by about 10⁻⁸, confirming implementation and convergence agreement. It does not mean the linear model is expressive enough.')),
      ...
      lab('training-lab', loc('学习率观察台', 'Learning-rate observation lab'), loc('只改变学习率和回放步数，区分稳定下降、缓慢下降与震荡。', 'Change only learning rate and replay steps to distinguish stable descent, slow descent, and oscillation.'), [loc('学习率', 'Learning rate'), loc('回放步数', 'Replay step')]),
  ...
  polynomial: {
    ...
    blocks: [
      ...
      formula('poly-formula', loc('对参数线性，对输入可以非线性', 'Linear in parameters, nonlinear in inputs'), '$$\hat y=b+w_1x+w_2x^2+w_3\sin(2\pi h/24)+w_4\cos(2\pi h/24)$$', loc('只要每个 w 都是一阶相加，模型仍是线性回归；非线性来自预先构造的特征。', 'The model remains linear regression because each w is added at first order; nonlinearity comes from engineered features.'), [
        { symbol: '$x^2$', meaning: loc('允许温度效应出现弯曲', 'allows curvature in the temperature effect') },
        { symbol: '$\sin,\cos$', meaning: loc('把 23 点和 0 点放回相邻位置', 'make hour 23 adjacent to hour 0 again') },
      ]),
...
8       122.028     219.046`, loc('次数升高持续改善训练 RMSE，却没有实质改善验证 RMSE。这正是“不预设特征越多越好”的例子。', 'Higher degree keeps improving training RMSE but does not materially improve validation RMSE. This is a concrete example of not assuming more features are better.')),
      ...
      lab('poly-lab', loc('特征阶段观察台', 'Feature-stage observation lab'), loc('切换预先声明的阶段，比较训练和验证 RMSE；若差距在 1% 内，保留较简单方案。', 'Switch predeclared stages and compare train and validation RMSE; keep the simpler design when results are within 1%.'), [loc('特征阶段', 'Feature stage'), loc('多项式次数', 'Polynomial degree')]),
  ...
  'model-limits': {
    ...
    blocks: [
      ...
      formula('coef-formula', loc('从标准化系数回到原始单位', 'Return standardized coefficients to raw units'), '$$w_j^{raw}=w_j^{scaled}/s_j,\quad b^{raw}=b^{scaled}-\sum_jw_j^{scaled}\mu_j/s_j$$', loc('连续变量除以训练标准差后进入模型，所以模型空间系数对应“一标准差变化”；换回原始单位需要除以 scale，并同步调整截距。', 'Continuous variables enter after division by the training standard deviation, so model-space coefficients represent one-standard-deviation changes. Raw-unit conversion divides by scale and adjusts the intercept.'), [
        { symbol: '$\mu_j$', meaning: loc('训练集第 j 列均值', 'training mean of feature j') },
        { symbol: '$s_j$', meaning: loc('训练集第 j 列标准差', 'training standard deviation of feature j') },
      ]),
...
weathersit_3   -54.580`, loc('连续列经过标准化，类别列是相对被删去基准类别的差异，因此这些数值不能直接跨编码方式比较。', 'Continuous columns are standardized and categorical columns are differences from omitted baselines, so these values cannot be compared without considering encoding.')),
      ...
      lab('coef-lab', loc('系数空间观察台', 'Coefficient-space observation lab'), loc('在模型空间与原始单位之间切换，先说明缩放和基准类别，再写出一句条件解释。', 'Switch between model space and raw units; state scaling and category baselines before writing a conditional interpretation.'), [loc('系数空间', 'Coefficient space'), loc('特征', 'Feature')]),
...
validation R²     0.401`, loc('验证误差明显高于训练误差，同时验证期目标水平更高。差距既包含泛化困难，也包含时间分布变化，不能简单归因于“训练轮数不够”。', 'Validation error is much higher while the later period also has greater demand. The gap combines generalization difficulty and temporal shift; it cannot be reduced to “not enough epochs.”')),
      ...
      lab('diag-lab', loc('诊断维度观察台', 'Diagnostic-view observation lab'), loc('切换预测—实际、残差—拟合值和具名案例；每次只回答一个诊断问题。', 'Switch among prediction-vs-actual, residual-vs-fitted, and named cases; answer one diagnostic question at a time.'), [loc('诊断维度', 'Diagnostic view'), loc('具名案例', 'Named case')]),
...
test evaluation count = 1`, loc('这个分数描述冻结线性方案面对更晚时间段的表现。它没有消除高峰低估和负预测等结构限制。', 'This score describes the frozen linear design on a later period. It does not remove structural limits such as peak underprediction and negative outputs.')),
      ...
      lab('reg-lab', loc('正则化观察台', 'Regularization observation lab'), loc('切换 OLS、Ridge、Lasso 和正则强度，观察相关特征的系数如何收缩，并始终对照验证 RMSE。', 'Switch among OLS, Ridge, Lasso, and regularization strength; inspect coefficient shrinkage while always checking validation RMSE.'), [loc('模型类型', 'Model type'), loc('正则强度', 'Regularization strength')]),
