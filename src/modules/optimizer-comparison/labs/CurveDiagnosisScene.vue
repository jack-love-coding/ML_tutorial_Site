...
<template>
  <section class="optimizer-scene" tabindex="0" :aria-label="text.title" @keydown.self.right.prevent="playback.step" @keydown.self.space.prevent="playback.step" @keydown.self.r.prevent="playback.reset">
    ...
    <p class="optimizer-scene__notice">{{ text.details }} {{ banknote.frozenSelection.optimizer }}: {{ text.learningRate }}={{ banknote.frozenSelection.learningRate }}, {{ text.batchSize }}={{ banknote.frozenSelection.batchSize }}, {{ text.epochs }}={{ banknote.frozenSelection.epochs }}. {{ text.split }}: {{ banknote.splitCounts.train }}/{{ banknote.splitCounts.validation }}/{{ banknote.splitCounts.test }}; {{ text.trainOnly }}; {{ text.testEvaluationCount }}={{ banknote.finalTestEvaluation.evaluationCount }}, {{ text.testUsedForSelection }}={{ banknote.finalTestEvaluation.selectionUsedTest ? text.yes : text.no }}.</p>
  </section>
</template>
