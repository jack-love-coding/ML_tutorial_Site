...
const selectedStep = ref(1e-6); const lastValid = ref(1e-6)
...
const copy = computed(() => props.locale === 'zh-CN' ? { title:'从单行残差到批量梯度', step:'中心差分步长 h', reset:'重置', table:'解析梯度与数值梯度', read:'柱形长度和表格同时对照解析梯度与中心差分；这里回放已发布的全批检查。', batch:'批量梯度是每一行 (p−y)x 的平均。', row:'单行贡献', finite:'有限差分检查' } : { title:'From one-row residual to batch gradient', step:'Centered-difference step h', reset:'Reset', table:'Analytic and numeric gradients', read:'Bar lengths and the table compare analytic and centered differences; this replays the published full-batch check.', batch:'The batch gradient averages each row contribution (p−y)x.', row:'One-row contribution', finite:'Finite-difference check' })
...
function reset(){ selectedStep.value=1e-6;lastValid.value=1e-6 }
