...
const mode = ref<'scratch'|'sklearn'|'l2'>('scratch'); const traceIndex=ref(0); const reducedMotion=window.matchMedia?.('(prefers-reduced-motion: reduce)').matches ?? false; const detailsOpen=ref(false)
...
function reset(){if(timer)clearInterval(timer);timer=undefined;mode.value='scratch';traceIndex.value=0;detailsOpen.value=false}
