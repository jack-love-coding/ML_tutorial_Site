...
const logit = ref(0); const lastValid = ref(0); const representation = ref<'probability' | 'odds'>('probability')
...
const copy = computed(() => props.locale === 'zh-CN' ? { title:'把分数压缩为概率', input:'logit z', probability:'概率', odds:'赔率', reset:'重置', table:'概率与赔率表', read:'曲线和表格同时保留；0 是概率 0.5 与默认阈值的桥梁。', invalid:'输入超出范围，已保留上一次有效值。' } : { title:'Compress a score into probability', input:'logit z', probability:'Probability', odds:'Odds', reset:'Reset', table:'Probability and odds table', read:'The curve and table remain together; zero is the bridge from probability 0.5 to the default threshold.', invalid:'The input was out of range; the last valid value remains.' })
function update() { if (model.value.inputValid) lastValid.value = model.value.lastValid; else logit.value = model.value.lastValid }
function reset() { logit.value=0; lastValid.value=0; representation.value='probability' }
