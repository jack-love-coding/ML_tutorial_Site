...
const rows = ref(1); const lastValid = ref(1); const reducedMotion = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches ?? false
...
const copy = computed(() => props.locale === 'zh-CN' ? { title:'从 Bernoulli 项到对数似然', rows:'累计行数', play:'播放累加', pause:'暂停', step:'单步', reset:'重置', table:'似然累加表', read:'乘积会越来越小；对数把乘法转换为有限的加法。', motion:'已遵循减少动态效果偏好：请使用单步按钮。' } : { title:'From Bernoulli terms to log-likelihood', rows:'Rows accumulated', play:'Play accumulation', pause:'Pause', step:'Step', reset:'Reset', table:'Likelihood accumulation table', read:'The product gets tiny; logarithms turn multiplication into finite addition.', motion:'Reduced motion is enabled: use Step instead.' })
...
function reset(){ if(timer)clearInterval(timer);timer=undefined;rows.value=1;lastValid.value=1 }
