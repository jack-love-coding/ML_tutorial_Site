...
const fifthChapter = moduleDefinition({
  ...
  sections: [
    section('sgd-full-batch-case', copy('Full batch：whole dataset average', 'Full Batch: Whole Dataset Average'), copy(md`full batch 每次更新前看完整训练集，把 whole dataset average 作为梯度。方向稳定，但数据大时每一步都很慢。它是理解 mini-batch 的基准。`, md`Full batch computes the gradient from the entire training set before each update. The direction is stable because it is the whole dataset average, but each step can be expensive when data are large. It is the baseline case: if all examples participate, the update direction is the average loss slope over the full dataset. Mini-batch and SGD change this cost by replacing the full average with a sampled estimate.`), { labIds: ['calculus-gradient-path-lab'] }),
