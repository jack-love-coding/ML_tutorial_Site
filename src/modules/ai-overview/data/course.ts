...
export const aiOverviewVisualCopy = {
  ...
  slope: loc('斜率 w', 'Slope w'),
  intercept: loc('截距 b', 'Intercept b'),
  singleStep: loc('前进一步', 'Single step'),
  autoRun: loc('自动运行', 'Auto run'),
  pause: loc('暂停', 'Pause'),
  speed: loc('速度', 'Speed'),
  residuals: loc('显示残差', 'Show residuals'),
  currentValue: loc('当前值', 'Current value'),
  history: loc('历史', 'History'),
  seed: loc('随机种子', 'Seed'),
  clusterCount: loc('簇数量 K', 'Cluster count K'),
  phase: loc('当前阶段', 'Current phase'),
  iteration: loc('迭代次数', 'Iteration'),
  initialization: loc('初始化中心', 'Initialize centers'),
  assignment: loc('把每个点分配给最近中心', 'Assign every point to its nearest center'),
  recomputation: loc('按簇均值重算中心', 'Recompute centers from cluster means'),
  converged: loc('分配稳定，算法收敛', 'Assignments are stable; the algorithm converged'),
  withinGroupDistance: loc('组内距离总和（越小越紧凑）', 'Within-group distance total (lower is tighter)'),
  explorationRate: loc('探索率', 'Exploration rate'),
  oneAction: loc('执行一次行动', 'Take one action'),
  oneEpisode: loc('训练一回合', 'Train one episode'),
  learningRate: loc('学习率', 'Learning rate'),
  discountFactor: loc('折扣因子', 'Discount factor'),
  policy: loc('当前策略', 'Current policy'),
  fullQTable: loc('完整 Q 表', 'Full Q table'),
  episode: loc('回合', 'Episode'),
  cumulativeReward: loc('累计奖励', 'Cumulative reward'),
  staticFallback: loc('关键状态（静态模式）', 'Key states (static mode)'),
  ...
  candidateHistory: loc('已访问候选历史', 'Visited candidate history'),
  visitSource: loc('访问来源', 'Visit source'),
...
export const aiOverviewLabCopy = {
  ...
  candidateVisitSources: {
    initial: loc('preset 初始', 'Preset initial'),
    manual: loc('手动参数', 'Manual parameters'),
    path: loc('确定性路径', 'Deterministic path'),
  },
  qUpdateTerms: {
    state: loc('状态 state', 'State'),
    action: loc('行动 action', 'Action'),
    oldValue: loc('旧 Q value', 'Old Q value'),
    reward: loc('奖励 reward', 'Reward'),
    nextBestValue: loc('下一状态最大 Q value', 'Next-state max'),
    target: loc('目标 target', 'Target'),
    correction: loc('修正量 correction', 'Correction'),
    newValue: loc('新 Q value', 'New Q value'),
  },
