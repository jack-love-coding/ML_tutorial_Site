...
静态图包含中文标题、轴标签和图例。为避免不同机器使用 PingFang、黑体或缺字 fallback 导致图像变化，阶段三提交一份来自 Google Fonts 固定提交的 Noto Sans SC variable TTF，并在绘图时固定 Regular（400）字重，随附 SIL Open Font License 与来源元数据。
...
3. Notebook 从八章母版生成，章节/正文/48 个来源 code cell 完整，双 ID 映射与类型契约一致。
