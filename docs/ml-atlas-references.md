...
| REF-SKLEARN-GETTING-STARTED | scikit-learn Getting Started | https://scikit-learn.org/stable/getting_started.html | Python 与 notebook 复现实验 | estimator API、fit/predict、模型训练最小工作流 | 在 NumPy/pandas 后阅读 | 用于校准第一个 sklearn 小模型 |
...
| REF-SKLEARN-CALIFORNIA-HOUSING | scikit-learn California housing dataset | https://scikit-learn.org/stable/modules/generated/sklearn.datasets.fetch_california_housing.html | 第一个端到端项目 | California housing 数据加载、字段说明、回归目标和 as_frame 用法 | 端到端项目读 CSV 前后阅读 | 房价预测项目的数据来源校准 |
