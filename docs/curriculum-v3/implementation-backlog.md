...
- Modules: `tokenization-language-modeling`, `attention-qkv-multihead`, `transformer-blocks`, `small-transformer-training`, `decoding-sampling`, `project-small-transformer`
- Exact exit criteria:
  - 能训练、验证并采样一个小型 Transformer。 / Train, validate, and sample from a small Transformer.

## `v3.7-reliable-llm-systems` — 可靠的 LLM 系统 / Reliable LLM Systems

- Modules: `llm-inference-context`, `peft-lora`, `retrieval-rag-systems`, `llm-evaluation-reliability`, `project-llm-application`
- Exact exit criteria:
  - 能用锁定评估证据审计适配与 RAG 系统。 / Audit adaptation and RAG systems with locked evaluation evidence.
