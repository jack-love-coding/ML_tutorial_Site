...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 25-01-01 | 01 | 1 | P25-SC1 | T-25-01 | Dataset bytes, schema, counts, split labels, and attribution are hash-checked before publication. | integration/drift | `node --test tests/numerical-methods-batch-4.test.ts` | ❌ W0 | ⬜ pending |
| 25-01-02 | 01 | 1 | P25-SC1–SC4 | T-25-01 / T-25-02 | Focused contract tests exist before generators and browser math are implemented. | test scaffold | `node --test tests/numerical-methods-batch-4.test.ts` | ❌ W0 | ⬜ pending |
| 25-02-01 | 02 | 2 | P25-SC1–SC3 | T-25-02 | Notebook publication rejects non-finite values, schema drift, hash drift, and partial output transactions. | integration | `python3 scripts/numerical-methods/generate-batch-4-notebook.py --check` | ❌ W0 | ⬜ pending |
| 25-02-02 | 02 | 2 | P25-SC2–SC3 | T-25-02 | Gradient, Armijo acceptance, terminal precedence, last-finite preservation, and baseline mapping match locked anchors. | unit/integration | `node --test tests/numerical-methods-batch-4.test.ts` | ❌ W0 | ⬜ pending |
| 25-03-01 | 03 | 3 | P25-SC3–SC4 | T-25-01 / T-25-03 | TypeScript parser and trainer reject malformed/non-finite inputs and match Notebook split/run anchors within declared tolerances. | unit/parity | `node --test tests/numerical-methods-batch-4.test.ts` | ❌ W0 | ⬜ pending |
| 25-04-01 | 04 | 4 | P25-SC4–SC5 | T-25-03 / T-25-04 | Existing routes, checkpoints, Progress keys, lab identities, and synthetic modes remain reachable and separately labeled. | source/component | `node --test tests/numerical-methods-batch-4.test.ts` | ❌ W0 | ⬜ pending |
| 25-04-02 | 04 | 4 | P25-SC4–SC5 | T-25-03 | Learner controls are bounded, explicit-run, resettable, and preserve the last finite state on failure. | component/browser | `node --test tests/numerical-methods-batch-4.test.ts` | ❌ W0 | ⬜ pending |
| 25-05-01 | 05 | 5 | P25-SC5 | T-25-04 | Illustration and all three videos consume locked Notebook values and retain poster, transcript, labels, hashes, and reduced-motion fallback. | asset/media | `node --test tests/numerical-methods-batch-4-manim.test.ts` | ❌ W0 | ⬜ pending |
| 25-06-01 | 06 | 6 | P25-SC5 | T-25-01–T-25-04 | Default/Pages builds, security audit, generator drift checks, and the 8-state browser matrix pass without runtime network access. | release/browser | `npm test && npm run build && npm run build:pages && npm run security:audit` | ✅ infrastructure | ⬜ pending |
