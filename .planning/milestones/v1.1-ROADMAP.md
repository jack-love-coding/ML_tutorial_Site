...
**Phases:** 26–31, including decimal Phases 27.1, 28.1, and 28.2
**Total Plans:** 37
**Requirements:** 43/43 complete

## Overview

v1.1 built one reproducible bilingual teaching corridor from loss values through
linear and logistic models to cost-aware classification decisions. The milestone
also delivered a leakage-safe tabular regression project, a six-part gradient
descent course, optimizer state and MLP transfer material, AI Foundations Part B,
and one shared five-step route while preserving all legacy routes and progress
stores.
...
- Phase 28.1: Gradient Descent Beginner Teaching Rebuild.
- Phase 28.2: Optimizer Principles, State, and MLP Transfer.
- Historical `27A`, `28A`, and `28B` labels in the original progress table refer
  to these canonical decimal phases.

**Key decisions:**

- Preserve canonical module IDs, deep links, checkpoints, and all existing
  progress-store bytes throughout the rebuild.
- Use frozen local datasets, executed bilingual Notebooks, deterministic
  TypeScript engines, hash-bound outputs, and base-safe media as one numerical
  teaching authority.
- Select operating thresholds on validation evidence and reveal the locked test
  result only once after the policy is frozen.
- Publish AI Foundations Part B only when its full eight-unit teaching corridor
  and all referenced resources pass stage-level release gates.

**Issues resolved:**

- Closed the missing Phase 28.1 UAT, verification, validation, and security
  provenance without fabricating evidence.
- Restored Phase 30 and Phase 31 summary/validation provenance and refreshed both
  verification reports after planning-only changes made them stale.
- Normalized phase identifiers, resolved stale ledgers, and reduced all open
  audit categories to zero.

**Issues deferred:**

- v1.0 historical provenance gaps and backend-dependent progress ownership remain
  explicitly deferred; they are not v1.1 regressions.
- AI Foundations Parts C and D remain behind whole-stage publication gates.

**Technical debt:**

- The existing Vite large-chunk advisory remains non-blocking and should be
  reviewed during a future performance-focused milestone.
- Production fairness monitoring and calibration-drift monitoring remain outside
  the scope of this static teaching site.
