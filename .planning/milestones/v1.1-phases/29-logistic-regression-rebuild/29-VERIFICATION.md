...
- **CI preview startup checked:** the browser runner strips terminal color controls before matching Vite's loopback URL, so the same ready server is recognized in plain local output and ANSI-decorated GitHub Actions output.
