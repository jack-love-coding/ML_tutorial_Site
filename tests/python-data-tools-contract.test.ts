...
test('planning state records shipped Python Data Tools stages and completed numerical-methods batches', async () => {
  ...
  const expectedStatus = '**Status:** Curriculum V3.0 blueprint and audit are complete. Python Data Tools Stages 1–4, the detailed minimum-mathematics, linear-algebra, calculus, probability routes, and Numerical Methods Batches 1–3 are complete. Numerical Methods Batch 4 is registered as Phase 25 and its context is ready for research and planning. Phase 24B Homepage Focus and Phase 24C Spine progressive disclosure remain paused.'
  const expectedFocus = '**Current focus:** Research and plan Phase 25 from the approved UCI Banknote logistic-regression context: stable BCE, feature scaling, fixed-step gradient descent, Armijo backtracking, stopping semantics, failure exits, and training-curve diagnosis. Do not implement before the plan and exact reproducible outputs are approved. Do not expand into Phase 24B Homepage Focus or Phase 24C Spine progressive disclosure.'
  const expectedNextCommand = 'Plan Phase 25 from the approved UCI Banknote logistic-regression context, lock exact clean-kernel outputs and implementation waves, then run plan review before implementation.'
  ...
  assert.match(state, /Do not expand into Phase 24B Homepage Focus or Phase 24C Spine progressive disclosure\./)
  ...
  assert.match(stageLines[4], /^5\. \*\*Consistency, browser, and build validation\*\* — Designed \/ ready for execution —/)
  assert.match(roadmap, /Stage 5 consistency and browser validation is designed and ready for execution/i)
  assert.match(roadmap, /four ordered plans cover learner-visible terminology/i)
