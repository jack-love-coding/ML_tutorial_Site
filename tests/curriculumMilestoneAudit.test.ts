...
test('milestone audit documents every completed phase and the current refactor state', () => {
  ...
  assert.match(stateSource, /Python Data Tools Stage 3 is not started/)
  ...
  assert.match(stateSource, /Current focus:\*\* Review the completed eight-chapter Python Data Tools Chinese master and scope Stage 3 Notebook\/output generation without changing the current `python-notebook` runtime lesson, route, checkpoints, or progress behavior\./)
})
