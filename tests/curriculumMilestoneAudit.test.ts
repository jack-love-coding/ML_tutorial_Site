...
test('milestone audit documents every completed phase and the current refactor state', () => {
  ...
  assert.match(stateSource, /Python Data Tools Stage 4 completed all 12 plans across 9 waves/)
  ...
  assert.match(stateSource, /Current focus:\*\* Research and plan Phase 25 from the approved UCI Banknote logistic-regression context/)
  assert.match(stateSource, /Stage 5 design is complete in four ordered plans/)
  assert.match(stateSource, /Numerical Methods Batches 1–3 are complete/)
  assert.match(stateSource, /Numerical Methods Batch 4 is registered as Phase 25 and its context is ready for research and planning/)
})
