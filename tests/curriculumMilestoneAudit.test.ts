...
test('milestone audit documents every completed phase and the current refactor state', () => {
  ...
  assert.match(stateSource, /Current focus:\*\* Discuss and contract Numerical Methods Batch 3/)
  assert.match(stateSource, /Stage 5 design is complete in four ordered plans/)
  assert.match(stateSource, /Numerical Methods Batches 1–2 are complete/)
  assert.match(stateSource, /Numerical Methods Batch 3 \(`finite-difference-methods`, `nonlinear-equations`\) is next for discussion and planning/)
})
