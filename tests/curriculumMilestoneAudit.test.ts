...
test('milestone audit documents every completed phase and the current refactor state', () => {
  ...
  assert.match(stateSource, /Phase 13 categorical vocabulary contract task lab merged/)
  assert.match(stateSource, /Phase 14 Data Quality Decision Record design is drafted for review/)
  assert.match(stateSource, /CategoricalVocabularyTaskLab/)
  assert.match(stateSource, /DataQualityDecisionRecordLab/)
  assert.match(stateSource, /Phase 14 Data Quality Decision Record/)
  assert.match(stateSource, /Current focus:\*\* Data-first lesson depth without adding backend, database, or durable progress scope/)
})
