...
test('milestone audit documents every completed phase and the current refactor state', () => {
  ...
  assert.match(stateSource, /Python Data Tools Stage 4 specification is approved and implementation is not started/)
  ...
  assert.match(stateSource, /Current focus:\*\* Discuss Stage 4 implementation decisions for English parity, the eight-chapter runtime migration, manifest-driven evidence, five formative exercises, legacy deep-link redirects, and checkpoint\/Progress compatibility before planning or runtime changes\./)
})
