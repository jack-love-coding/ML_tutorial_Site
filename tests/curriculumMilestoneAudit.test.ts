...
test('milestone audit documents every completed phase and the current refactor state', () => {
  ...
  assert.match(stateSource, /Python Data Tools Stage 2 is complete/)
  assert.match(stateSource, /Python Data Tools Stage 3 completed/)
  assert.match(stateSource, /Python Data Tools Stage 4 specification and implementation context are approved; implementation planning has not started/)
  ...
  assert.match(stateSource, /Current focus:\*\* Create the Stage 4 implementation plan for English parity, the eight-chapter paged runtime migration, manifest-driven outputs, five static teaching prompts, legacy deep-link redirects, and checkpoint\/Progress compatibility before runtime changes\./)
})
