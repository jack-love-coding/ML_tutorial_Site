...
test('math lab components and labs exist with expected contracts', () => {
  ...
  assert.match(matrixColumnSpaceSource, /function isTemporaryNumericDraft/)
  assert.match(matrixColumnSpaceSource, /isTemporaryNumericDraft\(rawValue\)/)
  assert.match(matrixColumnSpaceSource, /function commitControlInput/)
  assert.match(matrixColumnSpaceSource, /const parsedValue = Number\(rawValue\)/)
