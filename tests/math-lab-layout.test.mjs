...
test('math lab lazy routes are wired outside AlgorithmView', () => {
  ...
  assert.doesNotMatch(homeViewSource, /learningRouteSummaryModules/)
  assert.match(homeViewSource, /migrateLearningProgressV2/)
})
...
test('math lab components and labs exist with expected contracts', () => {
  ...
  assert.doesNotMatch(homeViewSource, /learningRoutes/)
  assert.doesNotMatch(homeViewSource, /learningRouteSummaryModules/)
  assert.doesNotMatch(homeViewSource, /modules\/math-lab\/data\/modules/)
  assert.match(homeViewSource, /migrateLearningProgressV2/)
  assert.match(homeViewSource, /selectCourseContinueTarget/)
  assert.match(homeViewSource, /loadCourseProgress/)
  assert.match(homeViewSource, /const legacyProgress = ref\(createDefaultLearningProgressV2\(\)\)/)
  assert.match(homeViewSource, /function refreshProgress\(\)/)
  assert.match(homeViewSource, /window\.addEventListener\('focus', refreshProgress\)/)
  assert.match(homeViewSource, /document\.addEventListener\('visibilitychange', handleVisibility\)/)
  assert.match(homeViewSource, /window\.addEventListener\('storage', handleStorage\)/)
  assert.match(homeViewSource, /onBeforeUnmount\(\(\) =>/)
  assert.match(homeViewSource, /window\.removeEventListener\('focus', refreshProgress\)/)
  assert.match(homeViewSource, /document\.removeEventListener\('visibilitychange', handleVisibility\)/)
  assert.match(homeViewSource, /window\.removeEventListener\('storage', handleStorage\)/)
  ...
  assert.match(modulePageSource, /migrateLearningProgressV2/)
