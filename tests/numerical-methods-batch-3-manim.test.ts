...
const metadataPath = resolve(root, 'public/manim/numerical-methods/batch-3-metadata.json')
...
const expectedScenes = [
  {
    ...
    durationSeconds: 78,
    storyboardCuts: [0, 8, 19, 32, 47, 60, 70, 78],
    posterSecond: 64,
  ...
  },
  {
    ...
    durationSeconds: 80,
    storyboardCuts: [0, 9, 20, 34, 48, 61, 71, 77, 80],
    posterSecond: 65,
...
test('Batch 3 metadata declares two complete deterministic teaching packages', () => {
  assert.equal(existsSync(metadataPath), true)
  const metadata = readMetadata()
  assert.equal(metadata.schemaVersion, 1)
  assert.equal(metadata.batchId, 'numerical-methods-batch-3')
...
test('six-role packages preserve depth, bilingual labels, and numerical boundaries', () => {
  const metadata = readMetadata()
  for (const scene of metadata.scenes) {
    ...
    assert.ok(prompt.split(/\s+/).length >= 1200)
  ...
  const finite = readFileSync(absolute(readMetadata().scenes[0]!.source), 'utf8')
  assert.match(finite, /0\.1630982544/)
  assert.match(finite, /2\.339e−12/)
  assert.match(finite, /6\.200e−5/)
  assert.match(finite, /最小步长 ≠ 最好步长/)
  assert.match(finite, /本节不训练模型/)
  const roots = readFileSync(absolute(readMetadata().scenes[1]!.source), 'utf8')
  assert.match(roots, /b\* = 0\.7302907403/)
  assert.match(roots, /37 次更新/)
  assert.match(roots, /39 次函数求值/)
  assert.match(roots, /Newton 步可能离开保护域/)
  assert.match(roots, /匹配一个平均值只解决这个标量方程/)
})

test('the two route chapters mount media matching Batch 3 metadata', () => {
  const metadata = readMetadata()
...
test('Batch 3 hashes and renderer check mode detect no package drift', () => {
  const metadata = readMetadata()
  ...
  const result = spawnSync('python3', ['scripts/manim/render_numerical_methods_batch_3.py', '--check'], {
    cwd: root,
    encoding: 'utf8',
  })
  assert.equal(result.status, 0, `${result.stdout}\n${result.stderr}`)
  assert.match(result.stdout, /Numerical-method Batch 3 Manim assets are in sync/)
})
