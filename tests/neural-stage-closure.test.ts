...
test('neural course renders full teaching before prediction and guided lab', () => {
  ...
  assert.doesNotMatch(source, /neural-mode-switch|mode === 'explore'/)
  assert.match(source, /<ChapteredMediaPlayer/)
  assert.match(source, /:chapter-markers="activeVisual\.chapterMarkers"/)
  assert.match(source, /:src="withPublicBase\(activeVisual\.assetPath\)"/)
