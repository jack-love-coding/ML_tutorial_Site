...
test('regression lab uses shared math and exposes complete labeled controls', () => {
  ...
  assert.match(regressionSource, /initializeRegressionSearch/)
  assert.match(regressionSource, /recordRegressionCandidate/)
  assert.match(regressionSource, /stepRegressionSearch/)
  assert.match(regressionSource, /setInterval\(step/)
  assert.match(regressionSource, /@input="recordManualCandidate"/)
  assert.match(regressionSource, /@change="applyPreset"/)
  assert.match(regressionSource, /<table[^>]*data-table="candidate-history"/)
  assert.match(regressionSource, /visit\.source/)
...
test('Q-learning lab uses episode utilities and exposes policy plus full table', () => {
  ...
  assert.match(qLearningSource, /cloneQTable\(qTable\.value\)/)
  assert.doesNotMatch(qLearningSource, /structuredClone\(qTable\.value\)/)
  ...
  for (const term of ['state', 'action', 'old-value', 'reward', 'next-best-value', 'target', 'correction', 'new-value']) {
    assert.match(qLearningSource, new RegExp(`data-update-term="${term}"`))
  }
  assert.match(qLearningSource, /lastUpdate\.nextBestValue/)
  assert.match(qLearningSource, /lastUpdate\.correction/)
